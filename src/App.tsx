...
// SISTEMA DEFENSIVO: Si los datos locales están corruptos, inicia de cero.
...
export const useStore = create<CadStore>()((set, get) => ({
  ...
  removeConstraint: (id) => set(state => ({ constraints: state.constraints.filter(c => c.id !== id) })),
               ...
               onContextMenu={(e)=>{
                 ...
                 if (handleShapes.length > 0) {
                   ...
                   handleShapes.forEach((s:any) => {
                     ...
                     if (!uniqueMap.has(rId)) uniqueMap.set(rId, { label: label.trim(), id: s.attrs.id });
                   });
              ...
              {contextMenu.items.some(i => i.id?.startsWith('pl') && i.id.split('_')[1] === it.id.split('_')[1]) && (
                 <div style={{padding: '6px 10px', cursor: 'pointer', color: 'black', background: '#eccc68', marginTop: '4px', borderRadius: '3px', fontSize: '0.8em', fontWeight: 'bold', textAlign: 'center'}}
                      onClick={() => {
                          ...
                          store.togglePlaneType(ex.id, planeId); setContextMenu(null);
                      }}>
...
export default function App() {
  ...
  return (
    <>
      <style>{`
        body { margin: 0; font-family: 'Segoe UI', sans-serif; background-color: #1e1e2f; color: #fff; overflow: hidden; }
        .app-layout { display: flex; height: 100vh; overflow: hidden; }
        .sidebar { width: 300px; background: #2a2a40; padding: 20px; display: flex; flex-direction: column; gap: 15px; overflow-y: auto; z-index: 100; box-shadow: 2px 0 10px rgba(0,0,0,0.5); flex-shrink: 0; }
        .main-area { flex: 1; overflow-y: auto; background: #151520; position: relative; padding-top: 60px; }
        .right-sidebar { width: 280px; background: #2a2a40; padding: 20px; display: flex; flex-direction: column; gap: 15px; overflow-y: auto; z-index: 100; box-shadow: -2px 0 10px rgba(0,0,0,0.5); flex-shrink: 0; border-left: 1px solid #444; }
        
        .top-navbar { position: absolute; top: 15px; left: 30px; display: flex; gap: 15px; z-index: 50; }
        
        .sheet-container { display: flex; flex-direction: column; gap: 40px; padding: 20px; align-items: center; transform-origin: top center; transition: transform 0.2s ease; }
        .page-sheet { background: white; width: ${PAGE_W}; min-height: 297mm; padding: 3mm; color: black; box-sizing: border-box; break-inside: avoid; display: flex; flex-direction: column; overflow: hidden; transition: width 0.3s ease; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
        ...
        .exercise-title { padding: 6px 10px; background: #f8f9fa; border-bottom: 1.5px solid black; font-weight: bold; word-wrap: break-word; line-height: 1.3; font-family: ${store.fontFamily}; font-size: ${store.fontSize}px; text-align: justify; outline: none; }
        .exercise-data { font-family: ${store.fontFamily}; font-size: ${store.fontSize - 1}px; padding: 4px 10px; text-align: justify; border-bottom: 1.5px dashed #ccc; font-weight: bold; outline: none; line-height: 1.3; word-wrap: break-word; }
        ...
        .btn-panel { background: #363654; color: white; border: none; padding: 10px; border-radius: 5px; cursor: pointer; font-weight: bold; transition: background 0.2s; display: flex; justify-content: center; align-items: center; }
        .btn-panel:hover { background: #00d2ff; color: #000; }
        .btn-panel:disabled { opacity: 0.5; cursor: not-allowed; }
        ...
        @media print { 
          ...
          .sheet-container { padding: 0 !important; gap: 0 !important; height: auto !important; overflow: visible !important; display: block !important; transform: none !important; } 
      ...
      <div className="app-layout">
        ...
        <div className="sidebar no-print">
          ...
          <div style={{ background: '#1e1e2f', padding: '15px', borderRadius: '8px' }}>
            ...
            <select value={store.pageSize} onChange={e => store.setPageConfig({pageSize: e.target.value as any})} style={{ width: '100%', padding: '8px', marginTop: '5px', background: '#363654', color: 'white', border: 'none', borderRadius: '4px', marginBottom: '10px' }}>
            ...
            <select value={store.fontFamily} onChange={e => store.setPageConfig({fontFamily: e.target.value})} style={{ width: '100%', padding: '8px', marginTop: '5px', background: '#363654', color: 'white', border: 'none', borderRadius: '4px', marginBottom: '10px' }}>
            ...
            <label style={{ color: '#00d2ff', fontWeight: 'bold', display:'block', marginBottom:'5px' }}>Tamaño Letra ({store.fontSize}px):</label>
            <input type="range" min="10" max="24" value={store.fontSize} onChange={e => store.setPageConfig({fontSize: Number(e.target.value)})} style={{ width: '100%', marginBottom: '15px' }} />
            ...
            {type === 'punto_coord' && (<div style={{marginTop: '10px'}}><label>Nº Puntos:</label><input type="number" value={ptCount} onChange={e=>setPtCount(Number(e.target.value))} min="1" max="10" style={{width:'100%', padding:'12px', fontSize:'16px', boxSizing:'border-box'}} /></div>)}
            {type === 'rectas' && (
              ...
              <div style={{marginTop: '10px'}}>
                ...
                {(abatElem === 'fig_reg' || abatElem === 'fig_irreg') && (
                  <div style={{marginTop:'10px'}}>
                    ...
                    <input type="number" value={abatLados} onChange={e=>setAbatLados(Number(e.target.value))} min="3" max="10" style={{width:'100%', padding:'12px', fontSize:'16px', marginTop:'5px', boxSizing:'border-box'}} />
                  </div>
                )}
                <div style={{marginTop:'10px'}}>
          ...
          <div style={{display: 'flex', gap: '5px', marginTop: 'auto'}}>
            <button onClick={store.saveData} style={{ flex: 1, background: '#ffa502', padding: '10px', border: 'none', fontWeight: 'bold', cursor: 'pointer', borderRadius: '5px' }}>💾 Guardar</button>
            <button onClick={store.loadData} style={{ flex: 1, background: '#1e90ff', padding: '10px', color: 'white', border: 'none', fontWeight: 'bold', cursor: 'pointer', borderRadius: '5px' }}>📂 Cargar</button>
          </div>
          <button onClick={store.downloadData} style={{ background: '#2ed573', padding: '10px', border: 'none', fontWeight: 'bold', cursor: 'pointer', borderRadius: '5px', marginTop:'5px' }}>⬇️ Descargar Lámina (.json)</button>
        ...
        <div className="main-area">
          ...
          <div className="top-navbar no-print">
            <div style={{display:'flex', gap:'8px', background:'#2a2a40', padding:'8px', borderRadius:'8px', border:'1px solid #444', boxShadow:'0 4px 6px rgba(0,0,0,0.3)'}}>
              <button className="btn-panel" onClick={store.undo} disabled={store.historyIndex <= 0} title="Deshacer (Ctrl+Z)" style={{padding:'8px 15px'}}>⮌ Deshacer</button>
              <button className="btn-panel" onClick={store.redo} disabled={store.historyIndex >= (store.history || []).length - 1} title="Rehacer (Ctrl+Y)" style={{padding:'8px 15px'}}>⮎ Rehacer</button>
              
              <div style={{width:'1px', background:'#444', margin:'0 5px'}}></div>
              
              <div style={{display:'flex', alignItems:'center', gap:'10px', padding:'0 10px'}}>
                <span style={{fontSize:'13px', fontWeight:'bold', color:'#00d2ff'}}>ZOOM:</span>
                <select value={store.sheetZoom} onChange={e => store.setPageConfig({sheetZoom: Number(e.target.value)})} style={{background:'#1e1e2f', color:'white', border:'1px solid #444', padding:'5px', borderRadius:'4px', outline:'none', fontWeight:'bold'}}>
          ...
          <div className="sheet-container" style={{ transform: `scale(${(store.sheetZoom || 100) / 100})` }}>
                    ...
                    {pageExs.map((ex) => (
                      <div key={ex.id} className="exercise-box" style={{ flexBasis: ex.w, minHeight: ex.h }}>
                        ...
                        <div className="no-print" style={{ display: 'flex', gap: '5px', padding: '4px 10px', background: '#f8f9fa', borderBottom: '1.5px solid #eaeaea' }}>
                          <button className="btn-mini" style={{ padding: '2px 6px', fontSize: '0.65rem' }} onClick={() => store.addFreeElement(ex.id, 'punto')}>+ Pto</button>
                          <button className="btn-mini" style={{ padding: '2px 6px', fontSize: '0.65rem' }} onClick={() => store.addFreeElement(ex.id, 'recta')}>+ Rct</button>
                          <button className="btn-mini" style={{ padding: '2px 6px', fontSize: '0.65rem' }} onClick={() => store.addFreeElement(ex.id, 'plano')}>+ Pln</button>
                        </div>
        ...
        {/* COLUMNA DERECHA NUEVA (RESTRICCIONES Y SELECCIÓN) */}
        <div className="right-sidebar no-print">
          <h3 style={{ color: '#00d2ff', margin: 0, fontSize:'16px', textTransform:'uppercase' }}>Restricciones Geométricas</h3>
          <p style={{fontSize:'12px', color:'#aaa', marginTop:'0', lineHeight:'1.4'}}>Para vincular, haz <b>Clic Izquierdo</b> sobre las líneas o trazas en el papel.</p>
          ...
          <div style={{background:'#1e1e2f', padding:'12px', borderRadius:'6px', border:'1px solid #444', minHeight:'120px', display:'flex', flexDirection:'column'}}>
            {(store.selectedElements || []).length === 0 && <span style={{color:'#64748b', fontStyle:'italic', fontSize:'13px', margin:'auto'}}>Ningún elemento seleccionado...</span>}
            
            {(store.selectedElements || []).map((item) => (
              <div key={item.id} style={{display:'flex', background:'#363654', padding:'8px 10px', borderRadius:'4px', marginBottom:'6px', borderLeft:'4px solid #ff9f43', fontWeight:'bold', fontSize:'14px'}}>
                {item.label}
              </div>
            ))}
            ...
            <div style={{marginTop:'10px'}}>
              ...
              <div style={{maxHeight:'200px', overflowY:'auto', display:'flex', flexDirection:'column', gap:'6px'}}>
                {store.constraints.map(c => (
                  <div key={c.id} style={{fontSize:'13px', background:'#363654', padding:'8px 10px', borderRadius:'4px', display:'flex', justifyContent:'space-between', alignItems:'center', border:'1px solid #555'}}>
                    <span style={{fontWeight:'bold'}}>{c.type === 'parallel' ? '║ Paralelismo' : '⟂ Perpendicularidad'}</span>
                    <button style={{background:'#ff4757', border:'none', color:'white', cursor:'pointer', borderRadius:'50%', width:'22px', height:'22px', fontWeight:'bold', display:'flex', justifyContent:'center', alignItems:'center'}} onClick={() => store.removeConstraint(c.id)}>X</button>
                  </div>
                ))}
              </div>
            </div>
